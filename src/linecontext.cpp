...
public:
    ...
    Symbol(Type type, const QString& name, const QString& shortName, int begin, int end, int indent)
...
public:
    void startClass(int lineNumber, int indent, const QString& name) {
        ...
        incomplete[indent] = Symbol(Symbol::Class, name, name, lineNumber, -1, indent);
    }

    void startMethod(int lineNumber, int indent, const QString& name, const QString& shortName) {
        flush(indent, lineNumber - 1);
        incomplete[indent] = Symbol(Symbol::Method, name, shortName, lineNumber, -1, indent);
    }
    ...
    QList<LineContextItem> context() const{
        ...
        for(int i=0;i<complete.size();i++) {
            ...
            std::sort(stack.begin(), stack.end(), [](const Symbol& s1, const Symbol& s2){
    ...
    QMap<int, Symbol> incomplete;
    QList<Symbol> complete;
...
class Reader {
public:
    Reader(QTextStream& stream) : stream(stream), lineNumber(0) {
...
void LineContext::parsePy(const QString &path) {
    ...
    mContext = helper.context();

    //mContext = helper.complete;
