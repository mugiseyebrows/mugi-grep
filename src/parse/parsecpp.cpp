...
QList<LineContextItem> ParseCpp::parse(const QString &path)
{
    QFile file(path);
    ...
    for(int i=0;i<par_op.size();i++) {
        ...
        if (short_name.contains(" ") || is_constructor_or_destructor_name(short_name)) {
            ...
            result.append(LineContextItem(full_name, short_name, begin, end));
        }
...
MatchingMap::MatchingMap(const QList<QPair<int, int> > &par_op, const QList<QPair<int, int> > &par_cl, const QList<QPair<int, int> > &br_op, const QList<QPair<int, int> > &br_cl) : par_op(par_op), par_cl(par_cl), br_op(br_op), br_cl(br_cl) {

}

int MatchingMap::matching(const QList<QPair<int, int> > &items, const QPair<int, int> &item, bool forward) {
...
int MatchingMap::indexOf(const QList<QPair<int, int> > &items, int pos) {
    for(int i=0;i<items.size();i++) {
        if (items[i].first == pos) {
            return i;
        }
...
int MatchingMap::matching(int pos) {
